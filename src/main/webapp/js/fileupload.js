...
fileupload['downloadPath'] = webfs+'/download/';
fileupload['delPath'] = webfs+'/delete/'
fileupload['queryPath'] = webfs+'/query/meta/';
...
var fileTrTmp='<tr>'
				+'<td>{{id}}</td>'
				+'<td><a target="_blank" href="'+fileupload.downloadPath+'{{id}}">{{name}}</a></td>'
				+'<td>{{size | sizeTransfer:"K"}}K</td>'
				+'<td>{{error?error:"上传成功"}}</td>'
				+'<td><a class="delbtn" delurl="'+fileupload.delPath+'{{id}}">删除</a></td>'
			+'</tr>';
...
fileupload.options = {
	...
	tabRenderTo:'', //上传表格，要渲染的目的地
		...
		$.each(options.result.files, function(index, file) {
			(function(){
				var tr=$(tr_render(file));
				tr.children('td:last').children('a').click({"options":options,"file":file}, fileupload_del);
				file_tbody.append(tr);
			})();
		});
		//回调函数
		options.afterDone(options.result.files);
	},
	...
	afterDone:function(file){//默认将file的ID值，附加到valueId组件的value中
		...
		for (var f in file){
			if(v.val()){
				v.val(v.val()+','+file[f].id);
			}else{
				v.val(file[f].id);
			}
		}
	},
	afterDel:function(file){
		var val=$(this.valueId).val();
		
		val=val.replace(","+file.id,"");
		val=val.replace(file.id,"");
...
function fileupload_del(e){
	...
	$.get(url,{},function(data,status){
		if(status=='success'){
			...
			alert('文件删除成功！');
			//回调接口函数
			e.data.options.afterDel(e.data.file);
		}else{
			alert('服务器异常!');
		}
	},'json');
...
fileupload.initUpdate=function(conf){
	...
	for(var i in ids){
		$.get(fileupload.queryPath+ids[i],{},function(data,status){
			if(status!='success'){
				alert("文件信息查询异常");
				return ;
			}
...
fileupload.initShow=function(conf){
	...
	for(var i in ids){
		$.get(fileupload.queryPath+ids[i],{},function(data,status){
			...
			var tr_render=template.compile(fileTrTmp);
			var tr=$(tr_render(data));
			tr.children('td:last').html('<a class="downloadbtn" target="_blank" href="'+data.downloadPath+data.id+'">下载</a>');
			fileTab.children('tbody').append(tr);
		},'json');
	}
}